...
window.addEventListener('error', (e)=>{
  ...
  try{ window.Swal ? Swal.fire({icon:'error', title:'Error en la página', text: msg}) : alert('Error: '+msg); }catch{}
...
const notify = {
  error:m=>window.Swal?Swal.fire({icon:'error',title:'Error',text:m}):alert(m),
  success:m=>window.Swal?Swal.fire({icon:'success',title:'Listo',text:m,timer:1400,showConfirmButton:false}):alert(m),
  toast:(m,icon='success')=>{
    if(window.Swal){ const T=Swal.mixin({toast:true,position:'top-end',showConfirmButton:false,timer:2000,timerProgressBar:true}); T.fire({icon,title:m}); }
    else console.log(icon.toUpperCase()+': '+m);
  },
  confirm:(m)=>window.Swal?Swal.fire({icon:'question',title:'¿Confirmar?',text:m,showCancelButton:true,confirmButtonText:'Sí',cancelButtonText:'Cancelar'}):Promise.resolve({isConfirmed:confirm(m)})
};
...
const initialProducts = [
  ...
  { id:3, name:"Vanilla Bean Crème Brûlée", price:7.00, stock:5,  img:"assets/images/image-creme-brulee-desktop.jpg" },
...
const availableStock = (p)=> Math.max(0, p.stock - (cart.find(i=>i.id===p.id)?.quantity || 0));
...
function saveCart(){ localStorage.setItem('desserts_cart_v1', JSON.stringify(cart)); }
  ...
  return `
    <li class="cart-li" data-id="${item.id}">
      ...
      <div>
        ...
        <div class="cart-meta muted">${item.quantity} × ${fmt.format(item.price)} = <strong>${fmt.format(lineTotal)}</strong></div>
...
function renderCart(){
  if(cart.length===0){
    cartList.innerHTML=''; $('#cart-empty').style.display='block'; cartTotal.textContent = fmt.format(0); return;
  }
...
list.addEventListener('click', (e)=>{
  ...
  const card = e.target.closest('[data-id]'); const id = Number(card.dataset.id);
  const product = products.find(p=>p.id===id);
  const qty = parseInt(card.querySelector('input[type="number"]').value,10) || 1;
  if(qty > availableStock(product)){ notify.error('No hay suficiente stock disponible.'); return; }
  const it = cart.find(i=>i.id===id); if(it) it.quantity += qty; else cart.push({ ...product, quantity: qty });
  saveCart(); renderCart(); renderProducts($('#search').value);
});

// ==== Eventos del carrito ====
cartList.addEventListener('click', (e)=>{
  const li = e.target.closest('.cart-li'); if(!li) return;
  const id = Number(li.dataset.id);
  if(e.target.closest('.rm')){ cart = cart.filter(i=>i.id!==id); saveCart(); renderCart(); renderProducts($('#search').value); return; }
  if(e.target.closest('.inc')){ const it = cart.find(i=>i.id===id); it.quantity++; saveCart(); renderCart(); renderProducts($('#search').value); return; }
  if(e.target.closest('.dec')){ const it = cart.find(i=>i.id===id); it.quantity = Math.max(1, it.quantity-1); saveCart(); renderCart(); renderProducts($('#search').value); return; }
});
cartList.addEventListener('change', (e)=>{
  const input = e.target.closest('.qtyInput'); if(!input) return;
  const li = e.target.closest('.cart-li'); const id = Number(li.dataset.id);
  const it = cart.find(i=>i.id===id); it.quantity = Math.max(1, Math.floor(Number(input.value)||1));
  saveCart(); renderCart(); renderProducts($('#search').value);
});

// ==== Controles ====
let t; $('#search').addEventListener('input', (e)=>{ clearTimeout(t); const v=e.target.value; t=setTimeout(()=>renderProducts(v),150); });
$('#resetBtn').addEventListener('click', ()=>{ $('#search').value=''; renderProducts(''); });

const IVA = 0.13;
$('#checkoutBtn').addEventListener('click', ()=>{
  if(cart.length===0){ notify.error('Tu carrito está vacío 🛒'); return; }
  const invoice = $('#invoice'); const ul = $('#invoice-items');
  ...
  const tax = +(subtotal*IVA).toFixed(2); const total = subtotal + tax;
...
$('#clearBtn').addEventListener('click', async ()=>{
  ...
  if(res.isConfirmed){ cart=[]; saveCart(); renderCart(); renderProducts($('#search').value); notify.success('Carrito vaciado'); }
});
$('#loginBtn').addEventListener('click', ()=> notify.toast('Función de inicio de sesión próximamente 🔐','info'));
